...
use near_sdk::{
    env, near_bindgen, serde_json::json, AccountId, Balance, BorshStorageKey,
...
pub struct TokenSeries {
    ...
    is_finalized: Vec<bool>,
    multiplier_value: Vec<u8>,
...
impl Contract {
    ...
    pub fn nft_create_series(
        ...
        let mut copies = token_metadata.copies.clone();
        let mut is_fin = Vec::new();
        let mut mul_val = Vec::new();

        while let Some(i) = copies {
            if i < 1 {
                copies = None;
            } else {
                is_fin.push(false);
        ...
        let abc = is_fin.clone();
        let def = mul_val.clone();
    ...
    pub fn nft_buy(
        ...
        let token_series = self.token_series_by_id.get(&token_series_id).expect("Error: Token series not exist");
        //TODO
        //
        //=> validate buying in starter store
        //1. get all tokens by series let tokens
        //    self.token_series_by_id.get(&token_series_id).unwrap().tokens;
        //2. loop through token_series
        //    tokens
        //    .iter()
        //    .map(|token_id| self.nft_token(token_id).unwrap())
        //    .collect()
        //3. make sure receiver_id hasnt minted this series
        //    assert!(receiver_id!=owner_id, "have minted this series")
        //
        let price: u128 = token_series.price.expect("Error: not for sale");
    ...
    pub fn nft_mint(
        ...
        let token_series = self.token_series_by_id.get(&token_series_id).expect("Error: Token series not exist");
        assert_eq!(env::predecessor_account_id(), token_series.creator_id, "Error: not creator");
        let token_id: TokenId = self._nft_mint_series(token_series_id, receiver_id.clone());
    ...
    fn _nft_mint_series(
        ...
        let mut token_series = self.token_series_by_id.get(&token_series_id).expect("Error: Token series not exist");
        ...
        let metadata = Some(TokenMetadata {
            ...
            extra: Some("{\"is_finalized\":false,\"multiplier_value\":1}".to_string()),
    ...
    pub fn nft_finalize(
        ...
        // let complementData: ComplementData = serde_json::from_str(&token_metadata.extra.to_string());

        // let metadata = Some(token_metadata);
        let metadata = Some(TokenMetadata {
            title: token_metadata.title,          // ex. "Arch Nemesis: Mail Carrier" or "Parcel #5055"
            description: token_metadata.description,    // free-form description
            media: token_metadata.media, // URL to associated media, preferably to decentralized, content-addressed storage
            media_hash: token_metadata.media_hash, // Base64-encoded sha256 hash of content referenced by the `media` field. Required if `media` is included.
            copies: token_metadata.copies, // number of copies of this set of metadata in existence when token was minted.
            issued_at: token_metadata.issued_at, // ISO 8601 datetime when token was issued or minted
            expires_at: token_metadata.expires_at, // ISO 8601 datetime when token expires
            starts_at: token_metadata.starts_at, // ISO 8601 datetime when token starts being valid
            updated_at: Some(env::block_timestamp().to_string()), // ISO 8601 datetime when token was last updated
            // extra: None, // anything extra the NFT wants to store on-chain. Can be stringified JSON.
            extra: Some("{\"is_finalized\":true,\"multiplier_value\":1}".to_string()),
            reference: token_metadata.reference, // URL to an off-chain JSON file with more info.
            reference_hash: token_metadata.reference_hash, // Base64-encoded sha256 hash of JSON from reference field. Required if `reference` is included.
        });
	...
	pub fn nft_get_series_single(&self, token_series_id: TokenSeriesId) -> TokenSeriesJson {
		...
		TokenSeriesJson{
            ...
            is_finalized: token_series.is_finalized,
            multiplier_value: token_series.multiplier_value,
		}
	}
...
mod tests {
    ...
    #[test]
    #[should_panic( expected = "Error: not for sale" )]
